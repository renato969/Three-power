...
<div class="paywall-overlay" id="recuperarOverlay">
  <div class="paywall-card" style="text-align:center;">
    ...
    <div style="font-size:12.5px; color:var(--dim); margin-bottom:16px; line-height:1.5;">Digite o email que você usou pra assinar. Se tiver uma assinatura ativa, mandamos um link pra entrar neste aparelho.</div>
...
(async function init(){
  ...
  document.getElementById('recuperarEnviar').addEventListener('click', async function(){
    ...
    let mensagem='Se esse email tiver uma assinatura ativa, você recebe um link de acesso em instantes.';
    try{
    ...
    }catch(e){ mensagem='Não consegui enviar agora. Tenta de novo em instantes.'; }
...
var MANIFEST_JSON = JSON.stringify({
  ...
  description: "Raciocínio, discernimento e influência, baseado no Eneagrama de Gurdjieff.",
  start_url: "/",
  scope: "/",
  display: "standalone",
...
async function handleMagicLink(request, env) {
  if (request.method !== "POST") return json({ error: "use POST" }, 405);
  const mensagemPadrao = "Se esse email tiver uma assinatura ativa, você recebe um link de acesso em instantes.";
  let body;
  try {
    body = await request.json();
  } catch (e) {
      ...
      if (env.BREVO_API_KEY && env.BREVO_FROM_EMAIL) {
        ...
        await fetch(BREVO_API_URL, {
          ...
          body: JSON.stringify({
            ...
            to: [{ email }],
            subject: "Seu link de acesso, Três Poderes",
            htmlContent: "<p>Clique no link abaixo para entrar no seu treino neste aparelho. Válido por 24 horas.</p><p><a href=\"" + link + "\">" + link + "</a></p>"
          })
        });
      }
    }
  } catch (e) {
    /* nunca revela nada na resposta, so tenta e segue */
  }
  return json({ ok: true, mensagem: mensagemPadrao });
}
